...
def returnTable(dictionary):
    ...
    formatted_df = df.style.background_gradient(cmap=cmap, subset=['Similarity (%)'], low=0, high=0.4)\
                        .set_table_styles([{'selector': 'th', 'props': [('background-color', '#dcdcdc'), ('color', 'black'),('border', '1px solid #ddd'), ('font-weight', 'bold'),('text-align', 'center'), ('vertical-align', 'middle')]}])\
    ...
    html_string = formatted_df.to_html()
    
    # Return the HTML string
    # return html_string.render()
    return html_string
